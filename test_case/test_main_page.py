...
class TestMainPage(object):
    ...
    def test_b4_set_schedule_auto(self, common_driver): # need a file to read all the schedule settings
        dustmagnet_detail_pages = DustMagnetDetailPages(common_driver)
        # check if the minute + 2 is equal greater than 60, like the time is 17:59
        if int(time.strftime("%M", time.localtime())) + 2 >= 60:
            start_minute = str(int(time.strftime("%M", time.localtime())) + 2 - 60)
            start_hour = str(int(time.strftime("%H", time.localtime())) + 1)    # if hour is 23 or 12h format 12
        else:
            start_minute = str(int(time.strftime("%M", time.localtime())) + 2)
            start_hour = time.strftime("%H", time.localtime())
        start_time = start_hour + ":" + start_minute
        # check if the minute + 2 is equal greater than 60, like the time is 17:59
        if int(time.strftime("%M", time.localtime())) + 2 >= 60:
            end_minute = str(int(time.strftime("%M", time.localtime())) + 4 - 60)
            end_hour = str(int(time.strftime("%H", time.localtime())) + 1)
        else:
            end_minute = str(int(time.strftime("%M", time.localtime())) + 4)
            end_hour = time.strftime("%H", time.localtime())
        end_time = end_hour + ":" + end_minute
        schedule_result = dustmagnet_detail_pages.set_new_schedule(start_time, end_time, "auto", [0, 1, 2, 3, 4, 5 ,6], "b4_schedule_auto" , "save")
        assert schedule_result

    @allure.story("test b4 check schedule auto on")
    @allure.severity(allure.severity_level.CRITICAL)
    def test_b4_check_schedule_auto_on(self, common_driver):   # need to read from a file with all the schedule settings
        # schedule_name = None
        schedule_mode = None
        schedule_hour = None
        ...
        for schedule_result in schedule_result_list:
            ...
            if schedule_name == "b4_schedule_auto":
                ...
                schedule_hour = int(schedule_result[-1].split(" ")[-1].split(":")[0]) # "Everyday hh:mm - hh:mm"
                schedule_minute = int(schedule_result[-1].split(":")[-1])  # the schedule end time minute

        if int(time.strftime("%M", time.localtime())) != schedule_minute:
            if int(time.strftime("%H", time.localtime())) > schedule_hour:
                wait_hour = int(time.strftime("%H", time.localtime())) - schedule_hour
            # wait_minute -1 means that the checking happens 1 minute before it ends
            wait_minute = wait_hour * 60 + schedule_minute - int(time.strftime("%M", time.localtime())) -1
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    def test_b4_check_schedule_auto_off(self, common_driver):  # need to read from a file with all the schedule settings
        # schedule_name = None
        schedule_mode = None
        schedule_hour = None
        schedule_minute = None
        ...
        if int(time.strftime("%M", time.localtime())) <= schedule_minute:
            #if int(time.strftime("%H", time.localtime())) == schedule_hour:
            wait_minute = schedule_minute - int(time.strftime("%M", time.localtime())) + 2
        else:
            if int(time.strftime("%H", time.localtime())) < schedule_hour:
                wait_hour = schedule_hour - int(time.strftime("%H", time.localtime()))
            wait_minute = wait_hour * 60 + schedule_minute - int(time.strftime("%M", time.localtime())) + 2
        time.sleep(60 * wait_minute)
        mode_info = dustmagnet_detail_pages.get_mode_info().split(" ")[0]
    ...
    def test_b4_set_schedule_manual2(self, common_driver):
        ...
        if int(time.strftime("%M", time.localtime())) + 2 >= 60:
            ...
            end_hour = time.strftime("%H", time.localtime())
        end_time = end_hour + ":" + end_minute
        schedule_result = dustmagnet_detail_pages.set_new_schedule(start_time, end_time, "manual", [0, 1, 2, 3, 4, 5 ,6], "b4_schedule_manual2" , "save", fanspeed="66.0", led="50")
        assert schedule_result
    ...
    def test_b4_check_schedule_manual2_on(self, common_driver):
        ...
        for schedule_result in schedule_result_list:
            ...
            if schedule_name == "b4_schedule_manual2":
                schedule_mode = schedule_result[1]
                schedule_hour = int(schedule_result[-1].split(" ")[-1].split(":")[0])
                schedule_minute = int(schedule_result[-1].split(":")[-1])  # the schedule end time minute
    ...
    def test_b4_check_schedule_manual2_off(self, common_driver):  # need to read from a file with all the schedule settings
        ...
        if int(time.strftime("%M", time.localtime())) <= schedule_minute:
            wait_minute = schedule_minute - int(time.strftime("%M", time.localtime())) + 1
        else:
            if int(time.strftime("%H", time.localtime())) < schedule_hour:
                wait_hour = schedule_hour - int(time.strftime("%H", time.localtime()))
            wait_minute = wait_hour * 60 + schedule_minute - int(time.strftime("%M", time.localtime())) + 1
